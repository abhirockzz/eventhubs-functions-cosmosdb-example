...
func main() {
	...
	go func() {
		for {
			//for i := 1; i <= 10; i++ {
			if producerOpen {
				oid := strconv.Itoa(int(rand.Int63n(10000)))

				//c := strconv.Itoa(i)
				order := Order{OrderID: oid, CustomerID: "customer-" + strconv.Itoa(rand.Intn(10)+1), Product: "product-" + strconv.Itoa(rand.Intn(10)+1)}
				b, err := json.Marshal(order)
				if err != nil {
					fmt.Println("Failed to marshal order info: ", err)
					continue
				}
				msg := &sarama.ProducerMessage{Topic: eventHubsTopic, Key: sarama.StringEncoder(oid), Value: sarama.ByteEncoder(b)}
				pt, off, err := producer.SendMessage(msg)
				if err != nil {
					fmt.Println("Failed to send msg:", err)
					continue
				}
				fmt.Printf("sent order ID %s to partition %d offset %d\n", oid, pt, off)
			}
